...
int main () {
	...
	case (2): {
		...
		if (pChoice==1) {
			...
			if (pChoice==3) {
				cout<<plot.text[49];
				character->decHealth(10);
				mon = Monster(30,30,30,"Carriage Driver");
				battRes = character->battle(mon);
				if (battRes==2) {
					if (!Death()) {
						save = 0;
	...
	case (4): {
		...
		if ((pChoice==1&&chance(50))||pChoice==2) {
			cout<<plot.text[98];
			mon = Monster(60,40,40,"Storm Mage");
			battRes=character->battle(mon);
			if (battRes==2) {
					if (!Death()) {
						save = 0;
